...
- (NSDate *)dateWithString:(NSString *)dateStr format:(NSString *)format{
    ...
    [dateFormatter setTimeZone:[NSTimeZone timeZoneForSecondsFromGMT:0]];//解决8小时时间差问题
...
{    
    if (self)
    {
        if (dict[@"price"] != nil) {
            ...
            NSMutableString *_time = [[NSMutableString alloc]initWithString:time];
            [_time insertString:@":" atIndex:2];
...
//
//            NSString * _time = [NSString stringWithFormat:@"%@ %@",todayDate,time];
//            NSDate *date = [self dateWithString:_time format:@"yyyy-MM-dd hhmm"];
//
//            NSTimeInterval timeInterval = [date timeIntervalSince1970];
//            _Date = [NSString stringWithFormat:@"%f",timeInterval * 1000 - (8 * 60 * 60 * 1000)];
            _Date = _time;
            _Open = @([dict[@"price"] floatValue]);
            _High = @([dict[@"price"] floatValue]);
            _Low = @([dict[@"price"] floatValue]);
            _Close = @([dict[@"price"] floatValue]);
            _Volume = [dict[@"volume"] floatValue];
        }else{
            NSString *time = dict[@"time"];
//            NSDate *date = [self dateWithString:time format:@"yyyy-MM-dd"];
//            NSTimeInterval timeInterval = [date timeIntervalSince1970];
//            _Date = [NSString stringWithFormat:@"%f",timeInterval * 1000 - (8 * 60 * 60 * 1000)];
            _Date = time;
