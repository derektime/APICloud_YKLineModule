...
static NSInteger const Y_StockChartSegmentOtherIndex = 4;
    ...
    if(!_indicatorView)
    {
        ...
        [titleArr enumerateObjectsUsingBlock:^(NSString*  _Nonnull title, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            [btn mas_makeConstraints:^(MASConstraintMaker *make) {
                ...
                make.bottom.equalTo(_indicatorView);
                if(preBtn)
                {
...
- (void)setSelectedBtn:(UIButton *)selectedBtn
{
    ...
    {
        if(selectedBtn.tag != Y_StockChartSegmentStartTag + Y_StockChartSegmentOtherIndex)
        {
            return;
        } else {
    ...
    } else if(selectedBtn.tag != Y_StockChartSegmentStartTag + Y_StockChartSegmentOtherIndex){
    ...
    if(_selectedIndex == Y_StockChartSegmentOtherIndex && self.indicatorView.frame.origin.x < 0)
    {
...
- (void)event_segmentButtonClicked:(UIButton *)btn
{
    ...
    if(btn.tag == Y_StockChartSegmentStartTag + Y_StockChartSegmentOtherIndex)
    {
        return;
    }
